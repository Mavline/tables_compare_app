...
- `docs/AI_AGENT_OPERATING_CONTRACT.md`, `docs/ARCHITECTURE.md`, `docs/BOM_DOMAIN.md`.
...
- After implementation, run the verification commands listed in `AOC-VERIFY-001`:
  - `npx tsc --noEmit`
  - `npm run build`
  - `npm test -- --watchAll=false`
  - For UI-affecting changes, start `npm start`, exercise the change in a real browser (Surfagent at `http://localhost:3456` when available), and save screenshots or DOM dumps.
